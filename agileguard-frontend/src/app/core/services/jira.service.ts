...
export class JiraService {
  ...
  // ── Story Editor typeahead lookups ────────────────────────────────────────
  ...
  createEpic(req: {
    ...
    priority?: string;
    businessLine?: string;
  }): Observable<ApiResponse<JiraIssue>> {
    return this.http.post<ApiResponse<JiraIssue>>(`${this.base}/epics`, req);
  }

  /** Inspects an existing epic — fetches child stories + runs gap detection on each. */
  inspectEpic(epicKey: string): Observable<ApiResponse<any>> {
    return this.http.get<ApiResponse<any>>(`${this.base}/epics/${epicKey}/inspect`);
  }

  /** Returns stories linked to an epic (lightweight, no gap analysis). */
  getEpicStories(epicKey: string): Observable<ApiResponse<JiraIssue[]>> {
    return this.http.get<ApiResponse<JiraIssue[]>>(`${this.base}/epics/${epicKey}/stories`);
  }

  // ── SAFe Sprint Report ────────────────────────────────────────────────────

  /** Generates the full SAFe sprint report for all feature teams. */
  getSprintReport(
    projectKey: string,
    sprintId?: number,
    pi?: string
    ...
    if (pi)       params = params.set('pi', pi);
    return this.http.get<ApiResponse<any>>(`${this.base}/sprint-report`, { params });
  }

  /** Returns all sprints for the sprint-selector dropdown in the SAFe report. */
  getSprintsForReport(projectKey: string): Observable<ApiResponse<any[]>> {
    return this.http.get<ApiResponse<any[]>>(
      `${this.base}/sprint-report/sprints`,
